...
# -------------------------------------------------------------------
# mock-cribl-start.sh — Start the Mock Cribl Stream service
# -------------------------------------------------------------------
...
usage() {
    cat <<EOF
Usage: mock-cribl-start.sh [OPTIONS]

Start the Mock Cribl Stream service via Docker Compose.
...
Examples:
  scripts/mock-cribl-start.sh -d          # Start in background
  scripts/mock-cribl-start.sh --build -d  # Rebuild and start
EOF
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        *)
            ...
            echo "Run 'mock-cribl-start.sh --help' for usage." >&2
            exit 1
            ;;
    esac
done
...
kill_port_process() {
    ...
    # Check if the process is our own Docker container — skip killing if so
    for pid in $pids; do
        local pname
