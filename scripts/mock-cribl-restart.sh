...
# -------------------------------------------------------------------
# mock-cribl-restart.sh — Restart the Mock Cribl Stream service
# -------------------------------------------------------------------
...
usage() {
    cat <<EOF
Usage: mock-cribl-restart.sh [OPTIONS]

Restart the Mock Cribl Stream service.
...
if [[ "$HARD" == true ]]; then
    ...
    exec "$SCRIPT_DIR/mock-cribl-start.sh" $START_ARGS
fi
